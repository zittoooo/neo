...
let failNum=0;              // 틀린 횟수
let watingSeconds; 			// 기억하도록 허용된 시간(10초)
...
let resultObj=null;			// 게임 진행 상태를 보려주는 태그 영역
let gameoverObj=null;			// 게임 진행 상태를 보려주는 태그 영역
...
function startGame() {
	...
	for(let i=0; i<NOOFANSWER;){ 
		let index = Math.floor(Math.random()*24); // 0~23사이의 정수 index 
...
function checkAnswer(e) { // e는 click 이벤트 객체
	let clickIndex = -1; // 초기 값 -1. 찾으면 발견한 인덱스 
